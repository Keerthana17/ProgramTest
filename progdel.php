...
if(isset($_POST['pgm-del']))
{
	$pname = mysql_real_escape_string($_POST['progname']);
	
	$pname = trim($pname);
		
	// Does program exists or not
	$query = "SELECT program_id FROM program WHERE program_name='$pname' AND program_user='$usnm'";
	$result = mysql_query($query) or die (mysql_error());
	
	$count = mysql_num_rows($result); // If program found, then delete
	
	if($count == 1){
		
		if(mysql_query("DELETE FROM program WHERE program_name='$pname' AND program_user='$usnm'"))
		{
			?>
			<script>alert('Program has been deleted ');</script>
			<?php
		}
		else
		{
			?>
			<script>alert('Cannot delete program');</script>
			<?php
		}		
	}
	else{
...
<head>
	...
	<title>Programming++ - Program Deletion</title>
					...
					<form method="post">
                        ...
                        <div class="row">
                            <div class="form-group col-xs-12">
                                <button type="submit" name="pgm-del" class="btn btn-success btn-lg">Submit</button>
                            </div>
                        </div>
                    </form>
